...
export const PhotosTab = (props: any) => {
    ...
    useEffect(() => {
        const getPhotosList = (async () => {
            const results = await getAllAsync(SQL_GET_SECTIONS(inspectionState.script_id!));
            let items: IItem[] = [];
            for (let i = 0; i < results.length; ++i) {
                ...
                items.push({
                    id: item.id,
                    name: item.name,
                    count: photosState.photos?.filter(p => p.sectionId === item.id).length || 0,
                });
            }
                ...
                renderItem={({ item, index, separators }) => (
                    <Surface key={item.name} style={styles.surface}>
                        <TouchableHighlight
                            key={item.id}
                            onPress={() => {
                                props.navigation.navigate("SectionPhotosList", { id: item.id })
                            }}
                            activeOpacity={0.6}
